(function () {
  ...
  _.map = function (collection, iterator) {
    ...
    _.each(collection, function (el) {
      // * iterator는 예를 들면 2*el의 리턴값이다.
      newArr.push(iterator(el));
    });
    return newArr;
  };
  ...
  _.reduce = function (collection, iterator, accumulator) {
    // * [1,2,3,5] , iterator();, accumulator;
    // * accumulator 값은 맨처음 값 collection [0] 리턴 값에 누적된 값에 리턴을 하고 또 리턴.
    ...
    if (accumulator !== undefined) {
      ...
      for (let i = 0; i < collection.length; i++) {
        container = iterator(container, collection[i]);
      }
    } else {
      container = collection[0];
  ...
  // *    //accumulator가 존재한다는 것이니까 기존 accumulator를 초기값으로 사용한다
  // *    for (let i = 0; i < collection.length; i++) {
  // *      accumulator2 = iterator(accumulator2, collection[i]);
  // *    }
  // *    //2. 초기값이 주어지지 않은 경우
  // *  } else {
  // *    accumulator2 = collection[0];
  // *    //초기값이 주어지지 않으면 배열의 0번째 요소부터 순회한다
  // *    for (let i = 1; i < collection.length; i++) {
  // *      accumulator2 = iterator(accumulator2, collection[i]);
  // *    }
  // *  }
  ...
  _.every = function (collection, iterator) {
    // *collection 안의 element가 iterator의 조건에 부합시 true가 반환되는 내장함수.
    let truty = true;
    // *iterator가 없는 경우
    if (iterator === undefined) {
      for (let el of collection) {
        //*엘리먼트값이 존재하면 false, truty 도 false;
        !el && (truty = false);
      }
      // *iterator가 있는 경우
    } else {
      for (let el of collection) {
        // * iterator의 조건에 충족한 el이 있으면 true
        !iterator(el) && (truty = false);
      }
    }
    // *저 경우를 빼고 나머지는 무조건 트루라는 조건을 주면 모든 조건을 충족한다. 그래서 위에 truty라는 변수를 선언.
    return truty;
  };
  ...
  _.delay = function (func, wait, ...el) {
    setTimeout(func, wait, ...el);
  };
  ...
  _.flatten = function (nestedArray, result) {
    // iresult에 변수에 할당된 것이 없다면 빈 배열을 만들어라.
    if (!result) {
      result = [];
    }
    // nestedArray가 배열이 아니면 result에 push한다
    if (!Array.isArray(nestedArray)) {
      result.push(nestedArray);
    } else
      for (let el in nestedArray) {
        _.flatten(nestedArray[el], result);
      }
    return result;
  };
